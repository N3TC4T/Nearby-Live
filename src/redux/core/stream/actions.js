...
export function getPosts(sectionIndex, startFrom) {
    return (dispatch) => {
        ...
        return AppAPI.stream.get({section, last: startFrom})
            .then((res) => {
                dispatch({
                    type: 'POSTS_UPDATE',
                    data: res,
...
export function getUserPosts(userID, startFrom) {
    return dispatch => AppAPI.people_info.get({pid: userID, section: 'posts', last: startFrom})
        .then((res) => {
            dispatch({
                type: 'POSTS_UPDATE',
                data: res,
...
export function featurePost(postID) {
    return dispatch => AppAPI.livestream.post({pid: postID, section: 'feature'})
        .then(() => {
            dispatch({
                type: 'POST_FEATURE',
                id: postID
...
export function getComments(postID) {
    return dispatch => AppAPI.livestream.get({pid: postID, section: 'comments'})
        .then((res) => {
            dispatch({
...
export function leaveComment(postID, comment) {
    return (dispatch) => {
        // for better user experience first we show comment as sending and then sent http request
        dispatch({
            type: 'COMMENTS_ADD',
            data: comment,
            id: postID
