    ...
    switch (action.type) {
        ...
        case 'MESSAGES_READ': {
            ...
            if (action.id && typeof action.id === 'string') {
                ...
                conversation.messages.filter({unread: true}).toModelArray().map((message) => {
        ...
        case 'MESSAGE_SEND': {
            ...
            if (action.id && typeof action.id === 'string' && action.data && typeof action.data === 'object') {
                if (Conversation.hasId(action.id)) {
                    ...
                    // we need to update msg and date for conversation so
                    // we don't need to update it from server on conversations list
                    conversation.update({
                        msg: action.data.body,
