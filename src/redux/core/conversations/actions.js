...
export function getConversations(startFrom) {
    return (dispatch) => {
        ...
        return AppAPI.conversations.get({start:startFrom, count:25, include:'text'})
            .then((res) => {
                dispatch({
...
export function deleteConversations() {
    return (dispatch) => {
        return AppAPI.conversations.post({action:'delete'})
            .finally(() => {
                dispatch({
...
export function getMessages(params) {
    return (dispatch) => {
        ...
        params.initialized ? _startFrom = params.last :null ;

        return AppAPI.conversations.get({ id: params.id , count:15 , start: _startFrom})
            .then((res) => {
                // we don't need to pass id from server , we create default uuid
                // we delete unread too cuz its not working fine
                res.map((obj) =>{delete obj.id ; delete obj.unread})
                dispatch({
...
export function sendMessage(message, pid) {
    return (dispatch) => {
        // for for better user experience first we show message as sending and then sent http request
        dispatch({
            type: 'MESSAGE_SEND',
            data: message,
            id: pid
