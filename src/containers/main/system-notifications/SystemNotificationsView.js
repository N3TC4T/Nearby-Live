...
class SystemNotificationsListingRender extends Component {
    ...
    LoadMore = () => {
        ...
        const startFrom = systemNotificationsListing[systemNotificationsListing.length - 1].id;
                ...
                <View style={[styles.message]}>
                    ...
                    <View style={[styles.header]}>
                        <View style={AppStyles.row}>
                            ...
                            <Text
                                style={[styles.time]}>{moment(notification.date).fromNow()} ago
                            </Text>
                        </View>
                    </View>
    ...
    render = () => {
        ...
        if (isRefreshing) {
            if (!systemNotificationsListing || systemNotificationsListing.length < 1) {
        ...
        if (!isRefreshing) {
            if (!systemNotificationsListing || systemNotificationsListing.length < 1 || error) {
                return (
                    <View style={[AppStyles.container]}>
                        <Error
                            text={ErrorMessages.notifications404}
                            tryAgain={() => { this.fetchNotifications(true); }}
                        />
                    </View>
                );
            }
